...
export default function FeatureShowcase() {
  ...
  return (
    <div className="flex flex-col md:flex-row rounded-3xl overflow-hidden shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] max-w-[650px] mx-auto w-[98%] mt-10 mb-12 border border-gray-200/80 bg-white">

      {/* ── Left sidebar ── */}
      <div className="md:w-[200px] md:shrink-0 bg-gray-50/50 px-2 py-4 border-b md:border-b-0 md:border-r border-gray-100 flex flex-col justify-center gap-2">
        {features.map((f, i) => (
          <button
            key={f.label}
            onClick={() => setActive(i)}
          ...
          >
            ...
            <div className="flex items-center justify-between mb-1">
              <span className={`text-[10px] font-bold tracking-widest ${active === i ? "text-[#2B64FD]" : "text-gray-400"}`}>
                {f.tag}
              </span>
              <span className={`text-xs font-mono font-bold ${active === i ? "text-[#2B64FD]/50" : "text-gray-300"}`}>
                0{i + 1}
              </span>
            </div>
            <p className={`text-[14px] font-bold leading-snug ${active === i ? "text-gray-900" : "text-gray-500"}`}>
              {f.label}
            </p>
          </button>
        ))}
      ...
      <div className="flex-1 bg-gradient-to-br from-[#F4F7FF] to-white text-gray-900 relative overflow-hidden">
        ...
        <div className="invisible select-none pointer-events-none px-12 py-8">
          <div className="flex items-center gap-4 mb-4">
            ...
            <h3 className="text-[22px] font-bold leading-tight">Automations &amp; Triggers</h3>
          </div>
          <p className="text-[13px] leading-relaxed mb-4 max-w-2xl">
            Personalize every customer journey automatically.
          </p>
          <ul className="space-y-2.5">
            <li className="flex items-start gap-3 text-[13px]"><span className="h-5 w-5 shrink-0" /><span>13+ condition types for full customization</span></li>
            <li className="flex items-start gap-3 text-[13px]"><span className="h-5 w-5 shrink-0" /><span>Auto-schedule email &amp; SMS campaigns</span></li>
            <li className="flex items-start gap-3 text-[13px]"><span className="h-5 w-5 shrink-0" /><span>Trigger responses based on user behavior</span></li>
          </ul>
        </div>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <div className="flex items-center gap-4 mb-4">
              ...
              <h3 className="text-[22px] font-bold tracking-tight text-gray-900 leading-tight">
                {features[active].headline}
              </h3>
            </div>
            <p className="text-gray-600 text-[13px] leading-relaxed mb-4 max-w-2xl">
              {features[active].description}
            </p>
            <ul className="space-y-2.5">
              {features[active].points.map((p) => (
                <li key={p} className="flex items-start gap-3 text-[13px] text-gray-700">
